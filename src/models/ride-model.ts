...
export const getRidesByPark = async (
  parkName: string
): Promise<{ operating: Ride[]; closed: Ride[] }> => {
  try {
    ...
    rides.forEach((ride) => {
      if (ride.location.toLowerCase() === parkName.toLowerCase()) {
        ...
        const isOpen =
          status === 'operating' || (status === '' && !ride.closing_date);
...
export const findBestParkByHeight = async (
  height: number
): Promise<{ park: string; count: number }> => {
  try {
    const rides = await getRidesData();
    const parkRideCounts: { [key: string]: number } = {};

    rides.forEach((ride) => {
      const maxHeightRestriction = extractMaxCmValue(
        ride.height_restriction || ''
      );

      if (
        (maxHeightRestriction === 0 || maxHeightRestriction <= height) &&
        ride.status.toLowerCase() === 'operating'
      ) {
...
export const findExtremeRides = async (): Promise<{
  fastest: Ride | null;
  highest: Ride | null;
}> => {
  try {
    ...
    rides.forEach((ride) => {
      if (ride.status.toLowerCase() === 'operating') {
        if (
          !fastestRide ||
          (ride.speed1_value &&
            ride.speed1_value > (fastestRide.speed1_value || 0))
        ) {
          fastestRide = ride;
        }
        if (
          !highestRide ||
          (ride.height_value &&
            ride.height_value > (highestRide.height_value || 0))
        ) {
          highestRide = ride;
        }
      }
